...
internal static class BitmapTranscoder
{
    ...
    private static unsafe void TranscodeFromYUVInternal(
       ...
       TranscodeFormats conversionStandard, Compression compression,
       byte* pFrom, byte* pTo)
    {
    ...
    private static unsafe void NV12_to_RGB24(
        ...
        byte* pFrom, byte* pTo)
    {
        int scatter = height / scatteringBase;
       ...
       byte* pFrom, byte* pTo)
    {
        ...
        {
            int y = ys * scatter;
            int myi = Math.Min(height - y, scatter);
            int c1, c2, d, e, cc1, cc2;

            for (int yi = 0; yi < myi; yi++)
            {
